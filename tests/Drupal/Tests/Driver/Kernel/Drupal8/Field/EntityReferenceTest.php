...
class EntityReferenceTest extends DriverFieldKernelTestBase {
  ...
  /**
   * Test referencing a user by name.
   *
   * Users don't have a label key, so their driver entity plugin has
   * to declare what field to identify them by.
   */
  ...
  /**
   * Test referencing a user by email address.
   *
   * Users don't have a label key, so their driver entity plugin has
   * to declare what field to identify them by.
   */
   ...
   *
   * Roles have string ids so can be referenced by label or id.
   */
  ...
  /*
  @todo It would be good to have case insensitive references, but it seems that
  ...
  $this->installEntitySchema('user_role');
  $role = Role::create(['id' => 'test_role', 'label' => 'Test role label'])->
  save();
  $this->fieldStorageSettings = ['target_type' => 'user_role'];
  $field = ['TEST_role'];
  $fieldExpected = [['target_id' => 'test_role']];
